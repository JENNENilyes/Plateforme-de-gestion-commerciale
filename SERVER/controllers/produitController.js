...
module.exports.updateQtePlus = async (req, res) => {

    if (!ObjectID.isValid(req.body._id))
      return res.status(400).send("ID unkown :" + req.body._id)
    const prod = await ProduitModel.findOne({_id: req.body._id})

    const STK= req.body.stock + prod.stock
    const updateRecord = {
...
module.exports.updateQteMoin = async (req, res) => {

    if (!ObjectID.isValid(req.body._id))
      return res.status(400).send("ID unkown :" + req.body._id)
    const prod = await ProduitModel.findOne({_id: req.body._id})

    const STK= prod.stock -req.body.stock
    const updateRecord = {
